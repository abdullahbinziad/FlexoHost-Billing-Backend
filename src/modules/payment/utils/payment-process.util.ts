...
export const paymentInitDataProcess = (data: IPaymentInitData & Record<string, any>): FormData => {
    ...
    //required - if applicable (for digital goods like hosting, might be stubbed)
    postData['shipping_method'] = data.shipping_method || 'NO';
    postData['num_of_item'] = data.num_of_item || 1;
    //optional
    ...
    postData['product_category'] = data.product_category || 'Hosting';
    postData['product_profile'] = data.product_profile || 'general';
    //optional
    ...
    for (const a in postData) {
        fdata.append(a, postData[a] !== undefined && postData[a] !== null ? String(postData[a]) : '');
    }
